...
def advanced_hybrid_fusion(
    ...
    meta_weight_model = np.where(
        ...
        np.where(
            ...
            0.96,
            np.where(np.abs(model_proba - 0.5) - np.abs(tds_proba - 0.5) > 0.2, 0.94, np.where(agreement_score < 0.1, 0.88, 0.92)),
        ),
    )
    ...
    safe_boost_zone = (
        (meta_ensemble > 0.58) & (meta_ensemble < 0.68) & (tds_proba > 0.68) & (model_proba > 0.55)
    )

...
from src.data import TONIoTPreprocessor, load_toniot_single_csv, mark_zero_day_attacks, zero_day_train_val_test_split
...
def main() -> None:
    ...
    df_raw = load_toniot_single_csv(config)
    df_raw, zero_day_types = mark_zero_day_attacks(df_raw, config)
    track(f"Observed held-out zero-day types: {zero_day_types[:10]}{'...' if len(zero_day_types) > 10 else ''}")
    ...
    if config.get("enable_qwenft", False):
        known_types = []
        if "type_norm" in df_raw.columns:
            attack_types_all = sorted(df_raw.loc[df_raw["is_attack"] == 1, "type_norm"].dropna().unique().tolist())
            zero_types_cfg = sorted({str(t).lower() for t in config.get("zero_day_attack_types", [])})
            known_types = [t for t in attack_types_all if t not in zero_types_cfg and t not in {"-", "normal"}]

        qwen = QwenFTDetector(config, known_types=known_types)
        qwen_proba_val, _ = qwen.predict_attack_proba(df_val_eval_raw, selected_features, batch_size=config.get("qwen_batch_size", 64))
        qwen_proba_test, _ = qwen.predict_attack_proba(df_test_eval_raw, selected_features, batch_size=config.get("qwen_batch_size", 64))
        ...
        thresholds["Hybrid3(TDS-Qwen3)"] = find_optimal_threshold(y_val_eval, hybrid_qwen_val, low=0.30, high=0.70, steps=81)

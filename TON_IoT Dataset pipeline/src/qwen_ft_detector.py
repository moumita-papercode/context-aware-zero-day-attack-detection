...
class QwenFTDetector:
    def __init__(self, config: Dict[str, Any], known_types: Optional[List[str]] = None):
        self.config = config
        self.known_types = known_types or []
        self.system_prompt = self.build_system_prompt(
            zero_day_types=[t.lower() for t in config.get("zero_day_attack_types", [])],
            known_types=[t.lower() for t in self.known_types],
        )
        self.tokenizer, self.model = self.load_qwen_ft()

    @staticmethod
    def build_system_prompt(zero_day_types: List[str], known_types: List[str]) -> str:
        return f'''You are a STRICT JSON-only TON-IoT Zero-Day Attack Detector.
...
1) Predict whether the record is "Normal" or "Attack".
2) Decide whether it is a "zero_day_attack" ("yes"/"no") based ONLY on the category rules below.

## ZERO-DAY SETUP (fixed)
- Normal label is: 0
- Attack label is: 1
- Attack category field name is: "type"
- ZERO_DAY_ATTACK_TYPES = {zero_day_types}
...
1) If "label" exists:
   - If label == 0 => predicted_value = "Normal"
...
1) If predicted_value == "Normal": zero_day_attack = "no"
...
- If missing info for predicted_value, default predicted_value to "Normal".
'''
    ...
    def make_single_row_record(self, row: pd.Series, row_id: int, extra_cols: List[str]) -> Dict[str, Any]:
        ...
        rec["label"] = self._safe_int_or_none(row.get("label")) if "label" in row else None
        rec["type"] = str(row.get("type", "-")) if "type" in row else "-"
    ...
    def predict_attack_proba(self, df_raw_eval: pd.DataFrame, feature_cols: List[str], batch_size: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        batch_size = batch_size or self.config.get("qwen_batch_size", 64)
        prompt_extra_cols = [c for c in feature_cols[: self.config.get("prompt_feature_limit", 12)] if c in df_raw_eval.columns]
        ...
        for start in tqdm(range(0, len(prompts), batch_size), desc="QwenFT inference"):
            ...
            for b in range(out.shape[0]):
                ...
                arr = self.safe_json_array_parse(text)
                ...
                pv = str(item.get("predicted_value", "Normal")).strip().lower()

...
class SignatureBasedDetector:
    def __init__(self, percentile: int = 95, min_violations: int = 1):
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        return (violations >= self.min_violations).astype(np.float32)
...
class AnomalyBasedDetector:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        n_comp = min(self.n_components, max(1, X_std.shape[1] - 1))
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        if self.nn_index is None or self.svd is None:
            return np.zeros(len(X), dtype=np.float32)
        ...
        ratio = d1 / d2_safe
        return (ratio > self.epsilon).astype(np.float32)


def train_traditional_detectors(X_train_sel: np.ndarray, y_train: np.ndarray, config: Dict) -> Tuple[SignatureBasedDetector, AnomalyBasedDetector]:
